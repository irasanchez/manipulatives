class Lexer {
    ...
    for (let i = 0; i < this.data.length; i++) {
      ...
      if (/\(/.test(part)) {
        ...
        if (/\)/.test(part)) {
          ...
          expression.content.push({ ...expressionPart, text: part, step: this.step });
    ...
    for (let i = 0; i < this.data.length; i++) {
      ...
      if (/\{/.test(part)) {
        ...
        if (/\}/.test(part)) {
          ...
          block.content.push({ ...blockPart, text: part, step: this.step++ });
        }
      }
    }
    return lexed[0];
  ...
  buildLex() {
    ...
    let keyWord = [{ text: this.data[0], isKeyWord: true, step: 1 }];
    this.step++
